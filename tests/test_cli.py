...
class ResolveCaldavUrlTest(unittest.TestCase):
    """`_resolve_caldav_url` overrides the CalDAV URL for `google`-backend
    accounts on the default root URL: the root returns 404 to PROPFIND,
    but the per-user `<root>/<email>/user/` URL works. Non-google
    accounts and google accounts on a custom URL pass through unchanged.
    """
    ...
    def _account(
        ...
        *,
        ...
        actual_credential: object = credential
        if actual_credential is None:
            actual_credential = GoogleCredential(client_id="cid", client_secret="cs")
        return AccountConfig(
            name="google",
            url=url if url is not None else GOOGLE_CALDAV_URL,
            username="",
            credential=actual_credential,  # type: ignore[arg-type]
            mirror_path=Path("/unused"),
            trash_retention_days=30,
            include=(re.compile(".*"),),
            exclude=(),
    ...
    def test_google_default_url_overridden_with_per_user_url(self) -> None:
        from chronos.authorization import Authorization

        bearer = mock.MagicMock()
        auth = Authorization(http_auth=bearer)
        with mock.patch(
            "chronos.cli.discover_google_user_email",
            return_value="me@example.com",
        ) as discover:
            url = cli._resolve_caldav_url(  # pyright: ignore[reportPrivateUsage]
                self._account(), auth
            )
        discover.assert_called_once_with(bearer)
        self.assertEqual(
            url,
            "https://apidata.googleusercontent.com/caldav/v2/me@example.com/user/",
        )

    def test_google_with_custom_url_passes_through(self) -> None:
        from chronos.authorization import Authorization

        custom = "https://my-google-proxy.example.com/dav/"
        url = cli._resolve_caldav_url(  # pyright: ignore[reportPrivateUsage]
            self._account(url=custom),
            Authorization(http_auth=mock.MagicMock()),
        )
        self.assertEqual(url, custom)

    def test_non_google_account_passes_through(self) -> None:
        from chronos.authorization import Authorization

        plaintext = PlaintextCredential(password="p")
        account = self._account(credential=plaintext, url="https://nextcloud.example/")
        url = cli._resolve_caldav_url(  # pyright: ignore[reportPrivateUsage]
            account, Authorization(basic=("u", "p"))
        )
        self.assertEqual(url, "https://nextcloud.example/")

    def test_discovery_failure_surfaces_as_caldav_error(self) -> None:
        from chronos.authorization import Authorization
        from chronos.caldav_client import CalDAVError
        from chronos.oauth import OAuthError
        ...
        with (
            mock.patch(
                "chronos.cli.discover_google_user_email",
                side_effect=OAuthError("HTTP 401"),
            ),
            self.assertRaises(CalDAVError) as ctx,
        ):
            cli._resolve_caldav_url(  # pyright: ignore[reportPrivateUsage]
                self._account(),
                Authorization(http_auth=mock.MagicMock()),
            )
        self.assertIn("Google account discovery failed", str(ctx.exception))
